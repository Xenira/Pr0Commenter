...
from keras.layers import Input, Dense, Activation, Dropout, Flatten, Reshape, RepeatVector, TimeDistributed, LeakyReLU, concatenate, GRU, RNN
import keras
...
title_ix_samples, title_unique_samples, past_samples, pred_samples = util.create_training_samples(
...
if CONTINUE_TRAIN:
    ...
    if USE_LSTM:
        ...
        past_dense = Embedding(
            comment_dict_size, EMBEDDING_SIZE, input_length=SEQ_SIZE)(past_in)
        x = concatenate([ctxt_dense, past_dense])
        x = Dropout(DO_RATE)(x)
        print(x)

        gru = GRU(200, return_sequences=USE_OUT_SEQ, use_bias=False)
        try:
            x = gru(x)
        ...
        if USE_OUT_SEQ:
            ...
            x = TimeDistributed(
                Dense(comment_dict_size, activation='softmax'))(x)
        else:
    ...
    model.compile(optimizer=Adam(lr=LR),
                  loss='sparse_categorical_crossentropy', metrics=[metric])
    print(model.summary())
    ...
    for j in range(NUM_MINI_EPOCHS):
        ...
        for i in range(start_i, end_i):
            ...
            batch_loss, batch_acc = model.train_on_batch(
                [title_batch, past_batch], [pred_batch])
            loss += batch_loss
            acc += batch_acc
            num += 1.0
